...
In the target project's `.claude/`: symlinks to `agents/`, `skills/`, `scripts/<stack>/` (one per active stack); optionally `overlays/` (when Azure SDK detected); `pipeline.config` (stacks + stack_paths + capabilities + pipeline_version + pipeline path + overlays); merged `settings.json` (PreToolUse hooks from all active adapters); `CLAUDE.md` and `ORCHESTRATOR.md` from templates; `local/` directory with project-specific overlay templates (`project-overlay.md`, `coding-standards.md`, `architecture-rules.md`, `review-criteria.md`); `tmp/` for recovery artifacts.
