...
8. **NO-CONTINGENT-FALLBACKS**: The task must not require "try command X; if it fails, run command Y instead" logic. Haiku is unreliable at chaining contingent fallbacks — it tends to report FAILURE after the first tool call instead of attempting the alternative. If verification genuinely needs a fallback path, assign the task to Sonnet.
